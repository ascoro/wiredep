  ...
  config.set('global-dependencies-sorted', filterExcludedDependencies(
    config.get('detectable-file-types').
      reduce(function (acc, fileType) {
		...
		var filter = filters[fileType]||['.' + fileType];
        if (!acc[fileType]) {
          acc[fileType] = prioritizeDependencies(config, filter);
        }
        return acc;
      }, {}),
    config.get('exclude')
...
function matchRestrictions(dependency, filters){

	for(var i=0;filters[i];i++){
		var filter = filters[i];
		if(filter[0] === '!'){
			filter = filter.substring(1);
			if(dependency.indexOf(filter)>-1){
				return false;
			}
		}else{
			if(dependency.indexOf(filter)===-1){
  ...
  var dependencies = mergeSort(
    $._.toArray(config.get('global-dependencies').get()).
      filter(function (dependency) {
		return matchRestrictions(dependency.main[0], fileType);
      }).
      filter(function (dependency) {
  ...
  return $._
    (dependencies).
      ...
      filter(function (main) {
		return matchRestrictions(main, fileType);
      });
}
